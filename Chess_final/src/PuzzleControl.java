...
public class PuzzleControl {
    ...
    public boolean next(){ //퍼즐 성립 최소 조건 6가지 확인 후 진행 여부 판단.
            ...
            for (int j = 0; j < 8; j++) { // 백에 승리조건에 불충족할 수 있는 기물들 수 체크
                if (this.puzzleFile.getPuzzleBoard().GBoard[i][j] != null) {
                    if(this.puzzleFile.getPuzzleBoard().GBoard[i][j].getPlayer().equalsIgnoreCase("w")) {
                        if (this.puzzleFile.getPuzzleBoard().GBoard[i][j].getPiece().equalsIgnoreCase("♚")) {
                            wKingCount = wKingCount + 1;
                        } else if (this.puzzleFile.getPuzzleBoard().GBoard[i][j].getPiece().equalsIgnoreCase("♝")) {
        ...
        for(int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) { // 흑에 승리조건에 불충족할 수 있는 기물들 수 체크
                if (this.puzzleFile.getPuzzleBoard().GBoard[i][j] != null) {
                    if (this.puzzleFile.getPuzzleBoard().GBoard[i][j].getPlayer().equalsIgnoreCase("b")) {
                        if (this.puzzleFile.getPuzzleBoard().GBoard[i][j].getPiece().equalsIgnoreCase("♚")) {
    ...
    public boolean diffcolor(){// 한쪽이라도 비숍이 다른 색 칸에 하나씩 있는지 확인
...
//    public boolean isCheckMate(Board chess){
//        int count = 0;
//        if (!this.isCheck(chess)) {
//            return false;
//        }else{
//            System.out.println(this.kingPos(chess).getMoves());
//            for(String t:chess.getwhitePieceList()){
//                int i = Controller.rankToInd(t);
//                int j = Controller.fileToInd(t);
//                ArrayList<String> pieceMoves = chess.GBoard[i][j].getMoves();
//                for(String h:this.kingPos(chess).getMoves()) {
//                        for(int x = 0; x<pieceMoves.size();x++){
//                            if(pieceMoves.get(x).equalsIgnoreCase(h)){
//                                 count++;
//                        }
//                    }
//                }
//            }
//            if (count > this.kingPos(chess).getMoves().size()) {
//                return true;
//            }
//        }
//    }
    ...
    public void setName() throws FileNotFoundException {
        ...
        while(this.control) {
            ...
            //제작자 이름, 메인 메뉴에서 사용자 이름 정하면 그거 받아올 것
            this.puzzleFile.setUserName(Main.userID);
            File file = new File("PuzzleMode/"+this.puzzleFile.getName()+"_"+this.puzzleFile.getTheme()+"_"+".txt");
            PrintStream fw = new PrintStream(new File("PuzzleMode/"+file.getName()));
            fw.println(this.puzzleFile.getName());
            fw.println(this.puzzleFile.getUserName());
            fw.println(this.puzzleFile.getTheme());
            fw.println(this.puzzleFile.getPlaydata());
            //아무도 깨지 않았기 때문에 특수문자인 ★로 표시
            fw.println("★");
            fw.println(" ");
            for(int r=0;r<this.reset.GBoard.length;r++)
            {
                for(int c=0;c<this.reset.GBoard[0].length;c++)
                {
                    if(this.puzzleFile.getPuzzleBoard().GBoard[r][c]==null)
                        fw.println("null");
                    else
                    {
